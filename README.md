...
| with-groups       | A comma or newline separated list of dependency groups to additionally include when selecting top-level packages/what gets synced. `main` names the project's own ungrouped dependencies. See "Dependency group selection" below. | `""` | no |
| without-groups    | A comma or newline separated list of dependency groups to exclude. `main` names the project's own ungrouped dependencies (`dev` additionally covers uv's legacy `tool.uv.dev-dependencies`). See "Dependency group selection" below. | `""` | no |
...
**Interaction with `create-issues`:** a failed `update-transitive` step files one managed issue, keyed by the reserved package name `transitive-dependencies` (reusing exactly the same create/update/close machinery documented below), and closes it automatically once the step later passes (or is unchanged) again.
